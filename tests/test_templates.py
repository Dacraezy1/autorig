...
class TestTemplateManager:
    ...
    def test_create_config_with_variables(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch('typer.confirm', return_value=True):
                TemplateManager.create_config_from_template("python", str(output_path), variables)
...
class TestTemplateValidation:
    ...
    def test_all_templates_have_descriptions(self):
        ...
        for template_name, template in TemplateManager.TEMPLATES.items():
            assert len(template["description"]) > 10, f"{template_name} description too short"
    ...
    def test_template_variables(self):
        ...
        for template_name in TemplateManager.TEMPLATES:
            ...
            if template_name in ["python", "web", "golang", "rust", "data-science"]:
                ...
                assert "@" in variables["email"], f"{template_name} email not a valid email"
    ...
    def test_template_config_validates(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            for template_name in TemplateManager.TEMPLATES:
                ...
                with patch('typer.confirm', return_value=True):
                    TemplateManager.create_config_from_template(template_name, str(config_path))
        ...
        for template_name in TemplateManager.TEMPLATES:
            ...
            for script in scripts:
                ...
                for pattern in dangerous_patterns:
                    assert pattern not in command, f"Dangerous pattern '{pattern}' in {template_name} template"
    ...
    def test_template_packages_are_common(self):
        ...
        for template_name in TemplateManager.TEMPLATES:
            ...
            for package in packages:
                ...
                assert all(c.isalnum() or c in "-._+" for c in package), f"Invalid package name: {package}"