...
@patch("autorig.installers.linux.shutil.which")
@patch("autorig.installers.linux.subprocess.run")
def test_linux_installer_apt_uninstall(mock_run, mock_which):
    # Setup
    mock_which.side_effect = lambda x: "/usr/bin/apt-get" if x == "apt-get" else None
    installer = LinuxInstaller()

    # Execute
    result = installer.uninstall(["vim", "git"])

    # Verify
    assert result is True
    mock_run.assert_called_once_with(
        ["sudo", "apt-get", "remove", "-y", "vim", "git"], check=True
    )
...
@patch("autorig.installers.linux.shutil.which")
@patch("autorig.installers.linux.subprocess.run")
def test_linux_installer_pacman_uninstall(mock_run, mock_which):
    # Setup
    mock_which.side_effect = lambda x: "/usr/bin/pacman" if x == "pacman" else None
    installer = LinuxInstaller()

    # Execute
    result = installer.uninstall(["vim"])

    # Verify
    assert result is True
    mock_run.assert_called_once_with(
        ["sudo", "pacman", "-Rns", "--noconfirm", "vim"], check=True
    )
...
@patch("autorig.installers.macos.shutil.which")
@patch("autorig.installers.macos.subprocess.run")
def test_macos_installer_uninstall(mock_run, mock_which):
    # Setup
    mock_which.return_value = "/usr/local/bin/brew"
    installer = MacOSInstaller()

    # Execute
    result = installer.uninstall(["wget"])

    # Verify
    assert result is True
    mock_run.assert_called_once_with(["brew", "uninstall", "wget"], check=True)
