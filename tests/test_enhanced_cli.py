    ...
    def test_bootstrap_with_template(self):
        ...
        with patch("typer.confirm", return_value=True):
            result = self.runner.invoke(
                app, ["bootstrap", "--path", str(config_path), "--template", "python"]
            )
...
class TestCLIIntegration:
    ...
    def test_template_integration_with_config_validation(self):
        ...
        for template_name in TemplateManager.TEMPLATES:
            ...
            with patch("typer.confirm", return_value=True):
                result = self.runner.invoke(
                    app,
                    ["template", "create", template_name, "--output", str(config_path)],
                )
    ...
    def test_verbose_output(self):
        ...
        with patch("typer.confirm", return_value=True):
            ...
            result_verbose = self.runner.invoke(
                app, ["apply", str(config_path), "--dry-run", "--verbose"]
            )

            # Test without verbose flag
            result_normal = self.runner.invoke(
                app, ["apply", str(config_path), "--dry-run"]
            )
    ...
    def test_profile_option(self):
        ...
        config_data = {
            "name": "Test Config",
            "profiles": {"test-profile": {"system": {"packages": ["test-package"]}}},
        }
        config_path = Path(self.temp_dir) / "test.yaml"
        config_path.write_text(yaml.dump(config_data))
        ...
        with patch("typer.confirm", return_value=True):
            result = self.runner.invoke(
                app,
                ["apply", str(config_path), "--profile", "test-profile", "--dry-run"],
            )
            assert result.exit_code == 0
