...
class ResourceMonitor:
    ...
    def get_average_usage(self) -> SystemResourceUsage:
        ...
        avg_disk = sum([r.disk_percent for r in self.resource_history]) / len(self.resource_history)
...
class StatusReporter:
    ...
    def report_status(self, show_resources: bool = True, show_config: bool = True):
        ...
        if show_config:
            ...
            table.add_row("System Packages", str(len(self.config.system.packages)), 
                         f"Packages: {', '.join(self.config.system.packages[:5])}{'...' if len(self.config.system.packages) > 5 else ''}")
            table.add_row("Git Repositories", str(len(self.config.git.repositories)), 
                         f"Repos: {len(self.config.git.repositories)}")
            table.add_row("Dotfiles", str(len(self.config.dotfiles)), 
                         f"Files: {len(self.config.dotfiles)}")
            table.add_row("Scripts", str(len(self.config.scripts)), 
        ...
        if show_resources:
            ...
            table.add_row("Memory Usage", f"{usage.memory_percent:.1f}%", "Current usage")
            table.add_row("Disk Usage", f"{usage.disk_percent:.1f}%", "Root partition")
            table.add_row("Network Out", f"{usage.network_sent:,} bytes", "Since monitoring started")
            table.add_row("Network In", f"{usage.network_recv:,} bytes", "Since monitoring started")
...
class OperationMonitor:
    ...
    def start_operation(self, operation_name: str):
        ...
        self.start_time = time.time()
        self.console.print(f"[bold blue]Starting operation:[/bold blue] {operation_name}")
        self.status_reporter.resource_monitor.start_monitoring()
        
    def report_progress(self, operation: str, current: int, total: int, 
                       description: str = ""):
        """Report progress of an operation."""
        percent = (current / total) * 100 if total > 0 else 0
        self.console.print(f"[cyan]{operation}:[/cyan] {current}/{total} ({percent:.1f}%) - {description}")
    ...
    def finish_operation(self, operation_name: str):
        ...
        self.console.print(f"[green]Operation '{operation_name}' completed in {duration:.2f} seconds[/green]")
