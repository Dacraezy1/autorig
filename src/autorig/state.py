...
class StateManager:
    ...
    def __init__(self, config: RigConfig, state_dir: str = "~/.autorig/state"):
        ...
        self.state_file = self.state_dir / f"{self._sanitize_filename(config.name)}.json"
    ...
    def create_rollback_point(self, operation: str, changes: List[Dict[str, Any]]):
        ...
        rollback_file = self.state_dir / f"{self._sanitize_filename(self.config.name)}_rollback_{operation}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    ...
    def get_latest_rollback(self, operation: str) -> Optional[Path]:
        ...
        rollback_files = list(self.state_dir.glob(f"{self._sanitize_filename(self.config.name)}_rollback_{operation}_*.json"))
        ...
        for change in changes:
            ...
            if action == "created_symlink":
                ...
                if previous_state.get("exists", False):
                    backup_path = Path(os.path.expanduser(previous_state.get("backup_path", "")))
...
class OperationTracker:
    ...
    def save_rollback_point(self):
        ...
        return self.state_manager.create_rollback_point(self.operation_name, self.changes)
