...
class DevContainerExporter:
    ...
    def _prepare_dotfiles(self):
        ...
        for df in self.config.dotfiles:
            ...
            source_abs = (self.original_config_dir / os.path.expanduser(df.source)).resolve()
            ...
            if source_abs.suffix == ".j2":
                ...
                try:
                    ...
                    self.renderer.render(str(rel_source), self.config.variables, dest_path)
    ...
    def _create_dockerfile(self):
        ...
        if self.config.git.repositories:
            ...
            for repo in self.config.git.repositories:
                target_path = repo.path.replace("~", "/home/vscode").replace("$HOME", "/home/vscode")
                # Ensure parent dir exists
                parent_dir = str(Path(target_path).parent)
                dockerfile_content.append(f"RUN mkdir -p {parent_dir} \")
                dockerfile_content.append(f"    && git clone -b {repo.branch or 'main'} {repo.url} {target_path} \")
                dockerfile_content.append(f"    && chown -R vscode:vscode {target_path}")
        ...
        if self.config.scripts:
             ...
             dockerfile_content.append("USER root") # Revert to root for end of file if needed
             dockerfile_content.append("")
