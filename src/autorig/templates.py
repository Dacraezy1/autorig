...
class TemplateManager:
    ...
    TEMPLATES = {
        ...
        "data-science": {
            ...
            "config": {
                ...
                "dotfiles": [
                    {
                        "source": "data-science/.gitignore.j2",
                        "target": "~/.gitignore"
                    },
                    {
    ...
    def list_templates(cls) -> None:
        ...
        for template_id, template_info in cls.TEMPLATES.items():
            table.add_row(
                template_id,
                template_info["name"],
                template_info["description"][:80] + "..." if len(template_info["description"]) > 80 else template_info["description"]
            )
    ...
    @classmethod
    def create_config_from_template(cls, template_name: str, output_path: str, variables: Dict[str, str] = None) -> None:
        ...
        if output_file.exists():
            if not typer.confirm(f"File {output_file} already exists. Overwrite?", default=False):
